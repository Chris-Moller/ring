...
test('Each new round generates a different polygon shape via resetForNextRound', () => {
  ...
  // Reset between rounds — this is where the new polygon is generated
  ...
  assert(!same, 'resetForNextRound generates different polygon vertices');
});
...
test('Countdown map matches round map (startRound preserves polygon)', () => {
  ...
  // Capture the polygon visible during the lobby/countdown
  ...
  // After startRound, vertices and centroid must be identical to the lobby polygon
  assert(
